...
def common(img_size):
    """
    basic augmentation for image: random crop and horizontalflip
    """
...
class Public_Dataset(ABC):
    ...
    def normalize(self, data_aug, img_size, mean , std):
        train_trans_list = []
        if data_aug: 
...
class MNIST_Dataset(Public_Dataset):
    def __init__(self,
        ...
        self.normalize(data_aug, img_size, mean, std)
...
def mnist(data_aug, img_size, data_dir='./data'):
    return MNIST_Dataset(data_aug, img_size, data_dir='./data').build()

def cifar10(data_aug, img_size, data_dir):
    return CIFAR10_Dataset(data_aug, img_size, data_dir='./data').build()

def cifar100(data_aug, img_size, data_dir):
    return CIFAR100_Dataset(data_aug, img_size, data_dir='./data').build()
